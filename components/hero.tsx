    ...
    <div>
      ...
      <div className="flex gap-3 justify-center">
        <motion.div animate={{ y: -20, opacity: 1 }} transition={{ duration: 0.75, type: "spring", stiffness: 100, delay: 0.2 }} initial={{ opacity: 0 }}>
          <Button className="shadow-sm hover:scale-105 p-6 flex gap-2" onClick={copyEmail}>
            {copied ? "Email copied!" : "Get in touch"}
            <IconMail />
          </Button>
        </motion.div>
        <motion.div animate={{ y: -20, opacity: 1 }} transition={{ duration: 0.75, type: "spring", stiffness: 100, delay: 0.3 }} initial={{ opacity: 0 }}>
          <a href="https://github.com/TimotheeRen" target="_blank"><Button variant="outline" className="hover:bg-accent-foreground shadow-sm p-6">
            GitHub
            <IconBrandGithub />
          </Button></a>
        </motion.div>
